...
from loopifi.downloader import download_video
from loopifi.loops import make_loops
from loopifi.parrots import random_parrot
...
def create_app():
    ...
    app.config.update(
        ...
        UPLOAD_FOLDER=os.path.join(os.path.dirname(__file__), "static", "res"),
        ...
        SQLALCHEMY_DATABASE_URI="mysql+pymysql://looper:nevergonnagiveyouup@localhost/loops",
    )
    ...
    @rq.job(timeout=500)
    def loopify(_id, path, url=None, sound=True):
        ...
        try:
            ...
            if url is not None:
                ...
                download_video(url, path, record.start_seconds, record.end_seconds)
            ...
            for r in result:
                video = Video(
                    webm_location=os.path.basename(r.webm_location),
    ...
    def browse():
        pagination = (
            Record.query.filter_by(done=True)
    ...
    def create():
        ...
        record = Record(
            start_seconds=startTotalSeconds, end_seconds=endTotalSeconds, url=url
        )
    ...
    def access(_id):
        ...
        if not record.done:
            ...
            return render_template(
                "waiting.html", job=record, position=position, parrot=random_parrot()
            )
    ...
    def stats():
        ...
        weekAgo = datetime(weekAgo.year, weekAgo.month, weekAgo.day)
        ...
        for dt in rrule.rrule(rrule.DAILY, dtstart=weekAgo, until=now):
            ...
            # 11:59PM of this day
            end = datetime(dt.year, dt.month, dt.day, hour=23, minute=59, second=59)
            ...
            records = (
                Record.query.filter(Record.created_at.between(start, end))
                .filter_by(done=True)
                .filter_by(failed=False)
                .all()
            )
            ...
            total_time = sum((r.finished_at - r.created_at).seconds for r in records)
